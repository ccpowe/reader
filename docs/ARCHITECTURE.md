...
主标签页只保留筛选、当前分类和滚动锚点等轻量控制状态；隐藏标签、阅读器下方页面及远离当前分类的列表视图会卸载。查询缓存仅保护当前页面恢复所需的身份隔离数据，并清理旧分类、旧搜索和旧排行变体；无限列表不会在用户阅读时截断，转为不活跃后只能删除当前锚点之后的尾部分页，确保向上阅读和单向游标仍然有效。列表位置、X 卡片成功译文的 200 片段 LRU 及这些页面控制状态都只存在于当前进程内存，X 译文按服务器、账户、目标语言、实际引擎指纹和原文匹配；应用重启或身份切换后重新请求，不建立客户端磁盘内容缓存。
...
[CLIBrowserSession](../backend/src/app/web_rule_agent/cli_browser.py) 为一次任务保留 agent-browser 0.37.1 与 Lightpanda 会话，跨模型等待、规则试跑和证据读取保留交互状态；结束、取消、超时或失去租约时回收任务进程。CLI 的 argv、会话、空配置、CDP 和 namespace 由宿主管理；工具响应加入证据 ID 后按完整 JSON UTF-8 限至 20 KiB，再从最终内容提取导航许可。规则执行／预检与 Worker 周期动态扫描继续使用 [web_crawl.py](../backend/src/app/ingestion/web_crawl.py) 的 Lightpanda 后端；静态页面保留 HTTP 路径，正常运行不自动回退 Chromium。共享配置为 `ingestion_browser_engine / ingestion_lightpanda_executable_path`，旧探索配置键仅为别名，旧 Chromium 配置明确报错。公网目的地、robots、TLS、重定向、请求和传输限制继续适用；安装、隔离、CDP 容量和环境检查见[部署说明](deployment.md#6-后端-web-规则-agent)。
